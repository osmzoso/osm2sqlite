...
def show_way(cur, way_id):
    ...
    cur.execute('''
        SELECT wn.node_id,wn.node_order,n.lat,n.lon
        FROM way_nodes AS wn
        LEFT JOIN nodes AS n ON wn.node_id=n.node_id
        WHERE wn.way_id=?
        ORDER BY wn.node_order''', (way_id,))
    for (node_id, node_order, lat, lon) in cur.fetchall():
...
def show_relation(cur, relation_id):
    ...
    cur.execute('SELECT key,value FROM relation_tags WHERE relation_id=?', (relation_id,))
    for (key, value) in cur.fetchall():
    ...
    for (part_relation_id,) in cur.fetchall():
