...
def draw_map(cur, lon, lat, zoomlevel, bbox_x, bbox_y, svgfile, show_unknown=True):
    ...
    # wgs84 boundingbox
    bbox_min_lon, bbox_min_lat = pixel_to_wgs84(bbox_min_x, bbox_min_y, pixel_world_map)
    bbox_max_lon, bbox_max_lat = pixel_to_wgs84(bbox_max_x, bbox_max_y, pixel_world_map)
    #
    ...
    for (ref, ref_id, key, value, layer, style, width, fill, stroke, dash) in cur.fetchall():
        ...
        for (lon, lat) in cur.fetchall():
            x, y = wgs84_to_pixel(lon, lat, pixel_world_map)
            x -= bbox_min_x
            y -= bbox_min_y
            y = bbox_y - y
            svg_path.append((x, y))
