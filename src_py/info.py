...
def show_relation(relation_id):
    ...
    db.execute('SELECT ref,ref_id,role FROM relation_members WHERE relation_id=? ORDER BY member_order', (relation_id,))
    for (ref, ref_id, role) in db.fetchall():
        print(f'relation {relation_id} member {ref} {ref_id} {role}')
    db.execute("SELECT relation_id FROM relation_members WHERE ref_id=? AND ref='relation'", (relation_id,))
